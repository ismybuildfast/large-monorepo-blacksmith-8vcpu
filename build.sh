...
marker_file=apps/crew/pages/benchmark-marker.tsx
...
# Check if turbo cache exists (indicates caching is working)
if [ -d "node_modules/.cache/turbo" ]; then
  cache_exists="true"
  cache_size=$(du -sh node_modules/.cache/turbo 2>/dev/null | cut -f1)
  echo "Turbo cache exists: $cache_exists (size: $cache_size)"
else
  cache_exists="false"
  cache_size="0"
  echo "Turbo cache does not exist"
fi
